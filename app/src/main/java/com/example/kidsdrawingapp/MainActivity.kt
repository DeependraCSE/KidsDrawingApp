...
class MainActivity : AppCompatActivity() {
                ...
                permissions.entries.forEach{
                    ...
                    if(permissionName == Manifest.permission.READ_EXTERNAL_STORAGE){
                        if (isGranted){
                            ...
                            Toast.makeText(this, "Oops you just denied the read the storage permission", Toast.LENGTH_LONG).show()
                        }
                    }
                    if(permissionName == Manifest.permission.WRITE_EXTERNAL_STORAGE){
                        if (isGranted){
                            Toast.makeText(this, "Permission granted, now you can write the storage file", Toast.LENGTH_LONG).show()
                            //selectImageFromGallery()
                        }else{
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val llPaintColor : LinearLayout = findViewById(R.id.llPaintColor)
        mImageButtonCurrentPaint = llPaintColor[1] as ImageButton
